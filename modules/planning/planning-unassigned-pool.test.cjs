...
function createPlanningEngineContext() {
  const engineContext = { console, Date, setTimeout, clearTimeout, window: { PLANNING_USE_MOCKS: false } };
...
test('admin take and finish is rendered only for admins in the real pool card and uses one transaction', () => {
  ...
  assert.match(planningUi, /Inicio real: \$\{escapePlanningHtml\(task\.inicioReal/);
