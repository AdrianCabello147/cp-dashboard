...
async function createPlanningTask(task) {
    ...
    try {
        savedTask = await savePlanningTask(taskToSave);
    } catch (error) {
...
async function savePlanningTaskChanges(taskId, task) {
    ...
    try {
        ...
        const updatedTask = updatePlanningTask(taskId, taskToSave, currentUser?.name || currentUser?.email || "Administrador");
...
async function deletePlanningTaskAction(taskId) {
    ...
    try {
        ...
        await persistPlanningTimelineEvent(taskId, {
            ...createPlanningTimelineEvent("delete", "Tarea eliminada"),
